...
Щоб перейти до цікавої частини, нам доведеться вивчити трохи математики, але наступний розділ написано так, щоб математика була відносно простою, і вона ілюструється рисунками. Використовуються лише найпростіші тригонометричні та експоненціальні властивості. Важливо зрозуміти базову математику, яку ми будемо використовувати у Python для пеленгації сигналу (DOA).

Розглянемо 1D трьохелементний рівномірно розміщену решітку:

.. image:: ../_images/doa.svg
   :align: center 
   :target: ../_images/doa.svg
   :alt: Рисунок, що показує напрямок прибуття (DOA) сигналу, який падає на рівномірно розташовану антенну решітку, показуючи кут напрямку та відстань між елементами або апертурами

У цьому прикладі сигнал падає з правого боку, тому спочатку він попадає правий елемент. Давайте обчислимо затримку між моментом, коли сигнал досягає цього першого елемента, і моментом, коли він досягає наступного елемента. Це можна зробити, вирішивши наступну тригонометричну задачу, спробуйте уявити, як цей трикутник був сформований на основі наведеного вище рисунку. Сегмент, виділений червоним, представляє відстань, яку сигнал має пройти після досягнення першого елемента, перш ніж досягти наступного.

.. image:: ../_images/doa_trig.svg
   :align: center 
   :target: ../_images/doa_trig.svg
   :alt: Триг, пов'язаний з напрямком прибуття (DOA) рівномірної решітки

Якщо ви пам'ятаєте з школи визначення, що таке сінус, косинус та тангенс, то ви побачете що в даному випадку нас цікавить "прилегла" сторона, а знаємо ми довжину гіпотенузи (:math:`d`), тому нам потрібно використовувати косинус:

.. math::
  \cos(90 - \theta) = \frac{\mathrm{adjacent}}{\mathrm{hypotenuse}}

Ми повинні знайти прилеглу сторону, оскільки саме вона покаже нам, яку відстань повинен пройти сигнал за проміжок часу між потрапляннями на перший і другий елемент :math:`= d \cos(90 - \theta)`. З тригонометричного рівнняня отримуємо це в :math:`= d \sin(\theta)`.  Однак це лише відстань, нам потрібно перетворити її на час, використовуючи швидкість світла: час, що минув :math:`= d \sin(\theta) / c` [секунди].  Це рівняння застосовується між будь-якими сусідніми елементами нашого масиву, також ми можемо помножити все це на ціле число для обчислення часу приходу між несуміжними елементами, оскільки вони розташовані рівномірно (ми зробимо це пізніше).  

Тепер пов'яжемо цю тригонометрії та обчислення за допомогою швидкості світла зі обробкою сигналів.  Позначимо наш передавальний сигнал у базовій смузі :math:`s(t)` і він передається на деякій несучій, :math:`f_c`, тому передавальний сигнал має вигляд :math:`s(t) e^{2j \pi f_c t}`.  Уявимо, цей сигнал потрапляє на перший елемент у момент часу :math:`t = 0`, що означає, що він потрапляє на наступний елемент через :math:`d \sin(\theta) / c` [секунд], як ми обчислили вище.  Це означає, що 2-й елемент отримує:
...
Нагадаємо, що коли у вас є часовий зсув, він віднімається від часу.
